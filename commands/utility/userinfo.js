...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const guildMemberRolesText = guildMember.roles.cache.map(r => r).slice(0,-1).join(' | ')
        ...
        const userInfoEmbed = new Discord.MessageEmbed()
            ...
            .addFields(
                {name: 'Registered Date', value: userRegistered, inline: true},
                {name: 'Joined Server Date', value: guildMemberJoined, inline: true},
                {name: 'Booster Date', value: guildMemberPremium ? guildMemberPremium : 'N/A', inline: true}
            )
            .addField(`Roles [${guildMemberRolesSize}]`, guildMemberRolesText ? guildMemberRolesText : 'No roles assigned.')
            .setTimestamp();
