...
module.exports = {
	...
	data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option => option.setName('int').setDescription('Enter a bet.')),
	...
	async execute(interaction) {
        ...
        if (bet){
            ...
            if (!regex.test(bet)) {
                return await interaction.reply('Invalid bet. Please try again!')
            }

            const balance = await dbFunctions.readBalance(interaction.guild.id, interaction.member.user.id);

            if (bet > balance) {
                return await interaction.reply('Your current balance is to low for this bet!');
            }
        ...
        const row = new MessageActionRow().addComponents([buttonHeads, buttonTails]);

        await interaction.reply({ content: 'Heads or Tails?', components: [row] });

        // Spawns a collector to collect respones from the buttons, user has 8 seconds to click the buttons.
        const collector = interaction.channel.createMessageComponentCollector({ componentType: 'BUTTON', time: 8000 });
		...
		collector.on('collect', async buttonInteraction => {
			if (buttonInteraction.user.id === interaction.user.id) {
                ...
                await buttonInteraction.reply({ content: message, components: [row] });
		...
		collector.on('end', async collected => {
            ...
            if (!choice) {
                return await interaction.editReply({ content: 'Plase click on the Heads or Tails buttons.', components: [] });
            }

			return await interaction.editReply({ content: 'This coinflip session has completed.', components: [] });
